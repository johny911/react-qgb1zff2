...
export default function MainAttendanceApp({ user, onLogout }) {
  ...
  const handleRowChange = (idx, field, val) => {
    const upd = [...rows]
    upd[idx][field] = val
    if (field === 'teamId') upd[idx].typeId = ''
    setRows(upd)
  }
  ...
  const deleteRow = (idx) => {
    const upd = [...rows]
    upd.splice(idx, 1)
    setRows(upd.length ? upd : [{ teamId: '', typeId: '', count: '' }])
  }
  ...
  return (
    ...
    >
      ...
      >
            ...
            {showPreview && (
              <div style={{ marginTop: 12 }}>
                ...
                <ul>
                  {rows.map((r, i) => {
                    const tName =
                      teams.find((t) => t.id.toString() === r.teamId)
                        ?.name || 'Unknown Team'
                    const tyName =
                      types[r.teamId]?.find(
                        (x) => x.id.toString() === r.typeId
        ...
        {/* <WorkReport> with correct prop name */}
        {screen === 'work' && (
          <WorkReport onBack={() => setScreen('home')} />
        )}

        {/* View existing work reports */}
        {screen === 'view-work' && (
